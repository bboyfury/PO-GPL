...
#SBATCH --job-name=eval_state_ae_d_coop
...
for env in "${environments[@]}"; do
    ...
    for exp in "${exp_names[@]}"; do
        for checkpoint in $(seq 0 $max_checkpoint); do
                ...
                echo " "
                echo "AE-GPL LBF"
                echo "exp number" $exp "checkpoint number" $checkpoint
                python test_ae_demo_r.py \
                    --lr=0.00025 \
                    --env-name="$env" \
                    --google-cloud="False" \
                    ...
                    --lrf-rank=6 \
                    --exp-name="$exp" \
                    --logging-dir="logs_${env_base_name}_run_ae_d" \
                    --saving-dir="param_${env_base_name}_run_ae_d"
