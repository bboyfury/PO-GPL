...
class GPLOnlyModelTraining(object):
    ...
    def eval_policy_performance(self, action_shape, model, cg_model, logger, logging_id):
        ...
        write_rewards_to_csv("train_average_return_ae.csv",self.config["env_name"],time.time(),self.config["exp_name"],self.config["load_from_checkpoint"],sum(avgs) / len(avgs))
        ...
        write_rewards_to_csv("eval_average_return_ae.csv",self.config["env_name"],time.time(),self.config["exp_name"],self.config["load_from_checkpoint"],sum(avgs) / len(avgs))
